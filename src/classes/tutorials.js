...
export function tutorial(v){
	if(v == 1){
		...
		extra_draw_stuff.push({type:"text", x:30, y:380, scroll:true, data:"Click on a hero to go to their inventory"})
		...
		shift(v1, 200, 200);
		var extra_draw_stuff = []
		extra_draw_stuff.push({type:"text", x:30, y:60, scroll:true, data:"This dungeon branches"})
		
		extra_draw_stuff.push({type:"text", x:30, y:150, scroll:true, data:"Click the first tile of a room to choose which path to take."})
		


		
		extra_draw_stuff.push({type:"text", x:30, y:800, scroll:true, data:"Alternatively, click the last tile of a room to iterate over the possible next paths"})
		extra_draw_stuff.push({type:"text", x:30, y:830, scroll:true, data:"This can only be done when NOT running a solution"})
		...
		var t1 =compile_level("", 2, ["v1", "w1"], [new dag(["v1"], []), new dag(["w1"], []) ], {"v1":[[["scroll_z"],[]]], "w1":[[["a"],["scroll_z"]]], }, ["scroll_z"], ["lever_1"],["a"],["a"])
